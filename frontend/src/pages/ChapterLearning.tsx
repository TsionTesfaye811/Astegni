...
const TABS = [
  ...
  ["textbook", "Textbook", BookOpen],
  ["videos", "Videos", Play],
  ["resources", "Resources", FolderOpen],
  ["quiz", "Practice Quiz", HelpCircle],
] as const;
...
export default function ChapterLearning() {
  ...
  const textbookFiles: LearningFile[] = [
    {
      title: `Grade ${grade} ${subjectDef.name} Textbook — Chapter ${chapterNumber}`,
      meta: "Official chapter textbook · 18 pages",
  ...
  const resourceFiles: LearningFile[] = [
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      <header className="border-b border-slate-200 bg-white">
        ...
        <nav className="mx-auto flex max-w-7xl gap-1 overflow-x-auto px-4 sm:px-6 lg:px-8">
          {TABS.map(([id, label, NavIcon]) => (
            <button key={id} onClick={() => switchTab(id)}
              className={`flex items-center gap-2 whitespace-nowrap border-b-2 px-4 py-3.5 text-sm font-semibold transition ${tab === id ? "border-[#2563EB] text-[#2563EB]" : "border-transparent text-slate-500 hover:text-slate-800"}`}>
      ...
      <main className="mx-auto grid max-w-7xl gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[1fr_270px] lg:px-8">
          ...
          {tab === "notes" && (
            <section className="rounded-2xl border border-slate-100 bg-white p-7 shadow-sm">
              <div className="mb-6 flex items-center justify-between gap-4">
                <div>
                  <span className="text-xs font-bold uppercase tracking-widest text-[#2563EB]">Page {notePage + 1} of 3</span>
                  <h2 className="mt-2 font-['Plus_Jakarta_Sans',sans-serif] text-2xl font-extrabold text-slate-900">{notes[notePage].title}</h2>
                </div>
                <span className="rounded-xl bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700">{chapterProgress.pagesRead.includes(notePage) ? "✓ Read" : "Reading"}</span>
              </div>
              <div className="h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full bg-[#2563EB]" style={{ width: `${((notePage + 1) / 3) * 100}%` }} /></div>
              <p className="mt-2 text-xs font-semibold text-slate-400">{notes[notePage].eyebrow}</p>
              <p className="mt-6 leading-7 text-slate-600">{notes[notePage].body}</p>
              <div className="mt-5 rounded-2xl border-l-4 border-blue-500 bg-blue-50 p-5 text-sm leading-6 text-slate-700">{notes[notePage].callout}</div>
              <ul className="mt-6 space-y-3">{notes[notePage].points.map(point => <li key={point} className="flex gap-3 text-sm leading-6 text-slate-600"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-emerald-500" />{point}</li>)}</ul>
              <div className="mt-8 flex flex-wrap items-center justify-between gap-3">
                <button disabled={notePage === 0} onClick={() => openNotes(notePage - 1)} className="flex items-center gap-2 rounded-xl border border-slate-200 px-5 py-3 text-sm font-bold text-slate-600 disabled:opacity-40"><ChevronLeft className="h-4 w-4" />Previous</button>
                {notePage < 2
                  ? <button onClick={() => openNotes(notePage + 1)} className="flex items-center gap-2 rounded-xl bg-[#2563EB] px-5 py-3 text-sm font-bold text-white">Next page <ChevronRight className="h-4 w-4" /></button>
                  : <button onClick={() => setTab("quiz")} className="flex items-center gap-2 rounded-xl bg-[#2563EB] px-6 py-3 text-sm font-bold text-white shadow-lg shadow-blue-200">Take Practice Quiz <ChevronRight className="h-4 w-4" /></button>}
              </div>
            </section>
          )}

          {tab === "textbook" && <FileLibrary title="Textbook" files={textbookFiles} onRead={setReader} onDownload={download} />}

          {tab === "videos" && (
            <section className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
              <div className="relative flex aspect-video items-center justify-center bg-slate-900">
                <img src="https://images.unsplash.com/photo-1635070041078-e363dbe005cb?w=900&h=500&fit=crop&auto=format" alt="Lesson preview" className="h-full w-full object-cover opacity-45" />
                <button className="absolute flex h-16 w-16 items-center justify-center rounded-full bg-[#2563EB] text-white shadow-xl"><Play className="h-7 w-7 fill-white" /></button>
              </div>
              <div className="p-6">
                <h2 className="font-bold text-slate-900">{chapterTitle}: guided video lesson</h2>
                <p className="mt-1 text-sm text-slate-500">Optional enrichment · 22 minutes</p>
              </div>
            </section>
          )}

          {tab === "resources" && <FileLibrary title="Additional Resources" files={resourceFiles} onRead={setReader} onDownload={download} />}

          {tab === "quiz" && (
            <section className="rounded-2xl border border-slate-100 bg-white p-7 shadow-sm">
              <h2 className="font-['Plus_Jakarta_Sans',sans-serif] text-2xl font-extrabold text-slate-900">Practice Quiz</h2>
              <p className="mt-2 text-sm text-slate-500">Answer all three questions to complete the chapter.</p>
              <div className="mt-7 space-y-5">
                {QUIZ.map((item, questionIndex) => (
                  <div key={item.question} className="rounded-2xl border border-slate-100 p-5">
                    <h3 className="mb-4 text-sm font-bold text-slate-800">
                      <span className="mr-2 inline-flex h-7 w-7 items-center justify-center rounded-lg bg-blue-100 text-blue-700">{questionIndex + 1}</span>
                      {item.question}
                    </h3>
                    <div className="grid gap-2 sm:grid-cols-2">
                      {item.options.map((option, optionIndex) => {
                        ...
                        return (
                          <button disabled={submitted} key={option}
                            onClick={() => setAnswers(current => current.map((answer, index) => index === questionIndex ? optionIndex : answer))}
                            className={`rounded-xl border px-4 py-3 text-left text-sm font-semibold ${correct ? "border-emerald-300 bg-emerald-50 text-emerald-800" : wrong ? "border-red-300 bg-red-50 text-red-800" : selected ? "border-blue-400 bg-blue-50 text-blue-800" : "border-slate-200 bg-slate-50 text-slate-700"}`}>
                            {option}
                          </button>
                        );
                      })}
              ...
              {!submitted
                ? <button disabled={answers.some(answer => answer === null)} onClick={submitQuiz} className="mt-6 w-full rounded-xl bg-[#2563EB] py-3.5 font-bold text-white disabled:bg-slate-300">Submit answers</button>
                : <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-6 text-center"><CheckCircle2 className="mx-auto h-9 w-9 text-emerald-500" /><h3 className="mt-2 font-bold text-emerald-900">Chapter complete!</h3><p className="mt-1 text-sm text-emerald-700">Score: {chapterProgress.score}/{QUIZ.length} · Your progress is now 100%.</p></div>}
            </section>
          )}
        ...
        <aside className="h-fit rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
          <h3 className="mb-4 text-sm font-bold text-slate-800">Learning path</h3>
          {learningPath.map((item, index) => (
            <button key={item.label}
              onClick={() => item.tab === "notes" ? openNotes(item.page ?? 0) : setTab(item.tab)}
              className={`mb-1 flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm ${tab === item.tab && (item.tab !== "notes" || notePage === item.page) ? "bg-blue-50 font-bold text-[#2563EB]" : "text-slate-600 hover:bg-slate-50"}`}>
              <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold ${item.done ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{item.done ? "✓" : index + 1}</span>
              {item.label}
            </button>
          ))}
        </aside>
      ...
      {reader && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" onClick={() => setReader(null)}>
          <div role="dialog" aria-modal="true" className="max-h-[85vh] w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl" onClick={event => event.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-slate-200 p-5">
              <div>
                <h2 className="font-bold text-slate-900">{reader.title}</h2>
                <p className="text-xs text-slate-500">{reader.meta}</p>
              </div>
              <button onClick={() => setReader(null)} className="rounded-xl p-2 text-slate-400 hover:bg-slate-100"><X className="h-5 w-5" /></button>
            </div>
            <div className="max-h-[60vh] overflow-y-auto whitespace-pre-line p-7 leading-7 text-slate-700">{reader.content}</div>
            <div className="border-t border-slate-200 p-4 text-right">
              <button onClick={() => download(reader)} className="inline-flex items-center gap-2 rounded-xl bg-[#2563EB] px-5 py-2.5 text-sm font-bold text-white"><Download className="h-4 w-4" />Download</button>
            </div>
          </div>
        </div>
      )}
...
function FileLibrary({ title, files, onRead, onDownload }: { title: string; files: LearningFile[]; onRead: (file: LearningFile) => void; onDownload: (file: LearningFile) => void }) {
  return (
    <section>
      <h2 className="mb-5 font-['Plus_Jakarta_Sans',sans-serif] text-2xl font-extrabold text-slate-900">{title}</h2>
      <div className="grid gap-4">
        {files.map(file => (
          <article key={file.title} className="flex flex-col gap-4 rounded-2xl border border-slate-100 bg-white p-5 shadow-sm sm:flex-row sm:items-center">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-blue-50"><FileText className="h-6 w-6 text-[#2563EB]" /></div>
            <div className="flex-1">
              <h3 className="font-bold text-slate-900">{file.title}</h3>
              <p className="mt-1 text-xs text-slate-500">{file.meta}</p>
            </div>
            <div className="flex gap-2">
              <button onClick={() => onRead(file)} className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-bold text-blue-700 hover:bg-blue-100">Read</button>
              <button onClick={() => onDownload(file)} className="flex items-center gap-2 rounded-xl bg-[#2563EB] px-4 py-2.5 text-sm font-bold text-white hover:bg-blue-700"><Download className="h-4 w-4" />Download</button>
            </div>
          </article>
        ))}
      </div>
